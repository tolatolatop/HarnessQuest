...
def convert_claude_jsonl(path: str | Path, *, project_name: str | None = None, user_email: str | None = None) -> dict[str, Any]:
    rows = _read_jsonl(Path(path))
    session_id = _first_str(rows, "sessionId") or Path(path).stem
    ...
    for row in rows:
        ...
        if row_type in {"system", "attachment", "file-history-snapshot", "mode", "permission-mode", "last-prompt"}:
            observations.append(
                {
    ...
    repository = cwd if cwd else None
    ...
    return {
        ...
        "project_name": project_name or _project_name(cwd),
...
def _handle_user_row(
    ...
    for item in content:
        ...
        output = _stringify(result_content)
        ...
        observations.append(_tool_observation(row, call, len(observations) + 1))
...
def _handle_assistant_row(
    ...
    for item in content:
        ...
        if item_type == "text":
            ...
            observations.append(
                {
                    "id": row.get("uuid") or tool_id or f"obs_{len(observations) + 1}",
                    "type": "tool",
...
def _classify_tool_result(
    ...
    if tool_name == "Bash":
        ...
        shell_commands.append(
            {
                "command": command or "",
                "exit_code": 1 if call.get("is_error") else 0,
                "output": output,
            }
        )
        ...
        file_edits.append(
            {
                "path": result_obj.get("filePath") or _input_path(call.get("input")),
                "change": result_obj.get("structuredPatch") or output,
            }
        )


def _tool_observation(row: dict[str, Any], call: dict[str, Any], index: int) -> dict[str, Any]:
    return {
        "id": row.get("uuid") or f"obs_tool_result_{index}",
        "type": "tool",
        "name": call["name"],
...
def _compact_row(row: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in row.items() if key not in {"message", "toolUseResult", "attachment", "snapshot"}}


def _first_content(conversation: list[dict[str, Any]], role: str) -> str | None:
    for item in conversation:
        if item.get("role") == role:
            return _str_or_none(item.get("content"))
    return None
...
def _last_prompt(rows: list[dict[str, Any]]) -> str | None:
    for row in reversed(rows):
        value = _str_or_none(row.get("lastPrompt"))
        if value:
            return value
    return None
